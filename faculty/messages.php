...
// students list (only those enrolled in subjects that this faculty teaches)
$students = [];
$stmt = $conn->prepare("
  SELECT DISTINCT s.user_id, s.name, s.roll_number, c.course_name, s.semester
  FROM faculty_subject fs
  JOIN enrollments e ON e.subject_id = fs.subject_id
  JOIN students s ON s.id = e.student_id
  LEFT JOIN courses c ON c.id = s.course_id
  WHERE fs.faculty_id = ?
  ORDER BY c.course_name, s.semester, s.roll_number
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'send') {
  ...
  // ensure receiver is in allowed student list
  $allowed = false;
  foreach ($students as $s) {
    if ((int)$s['user_id'] === $toUser) { $allowed = true; break; }
  }
  if (!$allowed) {
    flash_set('error', 'Not allowed to message this student.');
    header('Location: messages.php');
    exit;
  }
...
$stmt = $conn->prepare("
  SELECT m.id, m.subject, m.body, m.is_read, m.created_at,
         s.name AS sender_name, s.roll_number
  FROM messages m
  JOIN students s ON s.user_id = m.sender_user_id
...
<main class="py-4">
  <div class="container">
    <div class="row g-3">
      ...
      <div class="text-muted mb-3">Inbox and sending messages to your students.</div>
          ...
          <div class="card border-0 rounded-4 shadow-sm">
            <div class="card-body p-4">
              ...
              <form method="POST" class="row g-2">
                ...
                <div class="col-12">
                  <label class="form-label fw-semibold">To (Student) *</label>
                  <select class="form-select" name="to_user_id" required>
                    <option value="0">-- Select --</option>
                    <?php foreach ($students as $s): ?>
                      <option value="<?= (int)$s['user_id'] ?>" <?= $replyTo && (int)$replyTo['sender_user_id'] === (int)$s['user_id'] ? 'selected' : '' ?>>
                    ...
                    <?php else: ?>
                      <?php foreach ($inbox as $m): ?>
                        <tr class="<?= ((int)$m['is_read'] === 0) ? 'table-warning' : '' ?>">
                          <td><?= h($m['sender_name']) ?> (<?= h($m['roll_number']) ?>)</td>
